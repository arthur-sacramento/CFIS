...
include("db_connect.php");
    ...
    VALUES
        ...
        ('User2', 'hash1', '2024-01-31', 15),
        ('User1', 'hash2', '2024-01-01', 20),
        ('User2', 'hash2', '2024-01-31', 25)";
    ...
    // Query to calculate the sum of 'value' for each 'filehash' and organize by day
    $sumByDayQuery = "SELECT
        filehash,
        date,
    ...
    $result = $conn->query($sumByDayQuery);

    if ($result->num_rows > 0) {
        echo "<h2>Sum of 'value' for each 'filehash' organized by day:</h2>";
        echo "<table border='1'>";
    ...
    // Query to calculate the percentage growth or decrease for each 'filehash' based on the last day
    ...
    FROM
        ($sumByDayQuery) t1
    LEFT JOIN
        ($sumByDayQuery) t2 ON t1.filehash = t2.filehash AND t1.date = t2.date + INTERVAL 1 DAY
    ORDER BY
        t1.filehash, t1.date";
    ...
    if ($result->num_rows > 0) {
        echo "<h2>Percentage growth or decrease for each 'filehash' based on the last day:</h2>";
        echo "<table border='1'>";
        echo "<tr><th>Filehash</th><th>Date</th><th>Total Value</th><th>Percentage Change</th></tr>";
        while ($row = $result->fetch_assoc()) {
            echo "<tr><td>{$row['filehash']}</td><td>{$row['date']}</td><td>{$row['total_value']}</td><td>{$row['percentage_change']}</td></tr>";
        }
        echo "</table>";
    } else {
        echo "No results found";
